...
from backend.agents.agents import DEFAULT_AGENT, get_agent, get_all_agent_info
...
async def _handle_input(user_input: UserInput, agent: Pregel) -> tuple[dict[str, Any], UUID]:
    ...
    if user_input.agent_config:
        # Extract message_history if provided, as we'll handle it specially
        message_history = None
        if "message_history" in user_input.agent_config:
            message_history = user_input.agent_config.pop("message_history")
        ...
        # Update configurable with remaining agent_config items
    ...
    if interrupted_tasks:
        ...
        # Create initial message list with the current message
        messages = [HumanMessage(content=user_input.message)]
        
        # Add message history if provided in agent_config
        if message_history:
            history_messages = []
            for msg_data in message_history:
                # Convert message data to appropriate message types
                if msg_data.get("type") == "human" or msg_data.get("role") == "user":
                    history_messages.append(HumanMessage(content=msg_data.get("content", "")))
                elif msg_data.get("type") == "ai" or msg_data.get("role") == "assistant":
                    history_messages.append(AIMessage(content=msg_data.get("content", "")))
                elif msg_data.get("type") == "tool" or msg_data.get("role") == "tool":
                    # For tool messages, ensure they have the required fields
...
async def invoke(user_input: UserInput, agent_id: str = DEFAULT_AGENT) -> ChatMessage:
    ...
    # in that case.
    agent: Pregel = await get_agent(agent_id)
...
async def history(input: ChatHistoryInput) -> ChatHistory:
    ...
    agent: Pregel = await get_agent(DEFAULT_AGENT)
    try:
